...
- Recall the formula for covariance: $Cov(z_t, \varepsilon_{yt}) = E(z_t\varepsilon_{yt})-E(z_t)E(\varepsilon_{yt})$. Because $\varepsilon_{yt} \sim WN(0,\sigma_y^2)$, we obtain: $Cov(z_t, \varepsilon_{yt}) = E(z_t\varepsilon_{yt})$. 

- Next, expand the the expression for $y_t$ in the expression for $z_t$: $E(z_t \varepsilon_{yt}) = E[(-b_{21}[(b_{12}z_t + \gamma_{11}y_{t-1} + \gamma_{12}z_{t-1} + \varepsilon_{yt}] + \gamma_{21}y_{t-1}+\gamma_{22}z_{t-1} + \varepsilon_{zt})\varepsilon_{yt}] =*$. 
...
df <- df %>% mutate(dlip = log(indprod)-log(lag(indprod)), 
                    ...
                    s = r10 - tbill)
```
Do we need to check for stationarity here?
```{r}
...
# Page 310 and forward looks useful
