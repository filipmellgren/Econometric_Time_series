...
>>>>>>> cc1c2bf45d266441b0e0c5fce6b85082d31e571b
## 2
...
tmp <- df %>% dplyr::select(lip, urate, s) %>% dplyr::filter(row_number() > 1)
tmp <- as.matrix(tmp)
var_e <- VAR(tmp, p = 5, type = "none")
summary(var_e)
```
...
```{r}
tmp <- df %>% dplyr::select(s, dur, dlip) %>% dplyr::filter(row_number() > 1)
tmp <- as.matrix(tmp)
var_g <- VAR(tmp, p = 3, type = "none")
#summary(var_g)
