...
<<<<<<< HEAD
```{r}
#See steps in wikipedia, use book's notation: https://en.wikipedia.org/wiki/Vector_autoregression#Structural_vs._reduced_form
```
###c
...
For simplicity, we assume $a_{11} = a_{12} = 0$ which makes the determinant: 
$$ det(I-A_1) = 1-a_{22} = 0 \implies a_{22} = 1$$, with $a_{21}$ being any number.
...
```
The shock will never fade as the impact doesn't seem to return to zero and remains constant.
## 2
