...
# Select data used for input. Is the ordering here important? Possibly when we use irf with its cholesky decomposition later.
tmp <- df %>% dplyr::select(dlip, dur, s) %>% dplyr::filter(row_number() > 1)
tmp <- as.matrix(tmp)
# Var gives estimates of 3 OLS regressions. All the containing 3 lags from each variable.
var <- VAR(tmp, p = 3, type = "none")

# Necessary at this point?
irf(var, ortho = TRUE, n.ahead = 10)
