...
## 1 c 
Because of the cointegrating relationship, any deviations from the long run equilibrium will revert back, as the residual series follow a stationary process. The return to the equilibrium is descibed by the bivariate error correction system.
...
* $\Delta r_{Lt} = \alpha_L (r-L_{t-1}- \beta r_{St-1} -\mu)$

At time $t+1$ we obtain the expected change:

* $\Delta r_{St+1} = \alpha_S (r-L_{t}- \beta r_{St} -\mu) + a_{1,11} {\Delta r_{S t} + a_{1,12} {\Delta r_{L t} $
* $\Delta r_{Lt+1} = \alpha_L (r-L_{t}- \beta r_{St} -\mu) + a_{1,21} {\Delta r_{S t} + a_{1,22} {\Delta r_{L t}$
...
* $\Delta r_{St+1} = \alpha_S (r-L_{t}- \beta r_{St} -\mu) + a_{1,11} {\Delta r_{S t} + a_{1,12} {\Delta r_{L t} $
* $\Delta r_{Lt+1} = a_{1,21} {\Delta r_{S t}$
